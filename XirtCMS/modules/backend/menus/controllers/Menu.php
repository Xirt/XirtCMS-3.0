...
class Menu extends XCMS_Controller {
    ...
    public function move_up($id = -1) {
        ...
        foreach ($menus as $index => $menu) {
            ...
            if ($menu->get("ordering") < $this->menu->get("ordering")) {

                // Attempt to find best match
                if ($target == -1 || $menus[$target]->ordering <$menu->get("ordering")) {
                    $target = $index;
                }

                // Stop if best option found
                if ($menu->get("ordering") == $this->menu->get("ordering") - 1) {
                    break;
                }
        ...
        if ($target > -1) {
            ...
            $this->counterpart->load($menus[$target]->get("id"));
